...
async function invoke(request, socket){
    ...
    if(request.funcName == "checkPlate"){
        ...
        idCounter++ ;
        socket.emit('RESPONSE', {type: 'START', payload: `Transaction is successful.Check your Insurance id : LP_0` + idCounter });
        console.log('Transaction has been submitted');
   }
    else{
        await contract.submitTransaction('renewPlate', request.args[0]);
        socket.emit('RESPONSE', {type: 'START', payload: `Transaction is successful.Check your insurance id : ` + request.args[0] });
        console.log('Transaction has been submitted');
    }
    ...
    socket.on('REQUEST', (req) => {
        switch (req.action)
        {
            case "QUERY":
                ...
                break;
            case "RENEWAL":
                socket.emit('RESPONSE', {type: 'START', payload: `Request for RENEWAL of ${req.data.ID} received` });
                invoke(
                    { 
                        funcName: "renewPlate",
                        args: [req.data.ID]
                ...
                , socket);
                    break;
            case "CREATE":
                socket.emit('RESPONSE', {type: 'START', payload: `Request for CREATE received` });
                    invoke(
                        { 
                            funcName: "checkPlate",
